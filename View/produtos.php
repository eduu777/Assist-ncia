            ...
            <tbody>
              <?php foreach ($totalprodutos as $p) : ?>
                <tr>
                  ...
                  <td><?=$p['modelo']?></td>
                  <td><?=$p['fabricante']?></td>
                  <td><?php $daouser = new UsuarioDAO(); $client = $daouser->carregar($p['cod_usuario']); echo($client[0]['nome']);?></td>
                  <?php $daoorc_prod = new Orcamento_prodDAO(); $orc_prod = $daoorc_prod->carregar_prod($p['cod']); $daoorc = new OrcamentoDAO(); $orc = $daoorc->carregar($orc_prod[0]['cod_orcamento']); ?>
                  <td><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#visualizarOrc" data-bs-whatever="<?=$orc[0]['cod']?>" data-bs-whateverdata="<?=$orc[0]['data']?>" data-bs-whateverhora="<?=$orc[0]['hora']?>" data-bs-whateverstts="<?=$orc[0]['stts']?>" data-bs-whateverclliente="<?=$client[0]['nome']?>" data-bs-whateverdesc="<?=$orc[0]['descricao']?>" data-bs-whatevervalor="<?=$orc[0]['valor']?>">
                    <i class="bi bi-eye"></i>
                  </button></td>
                  <td><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#responderModal" data-bs-whatever="<?=$p['cod']?>" data-bs-whatevermodelo="<?=$p['modelo']?>" data-bs-whateverfab="<?=$p['fabricante']?>" data-bs-whateversttsp="<?=$p['stts']?>" data-bs-whateverestado="<?=$p['estado']?>" data-bs-whateverfoto="<?=$p['foto']?>" data-bs-whateverdescricao="<?=$p['descricao']?>">Editar</button></td>
                  <!--<td><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#deletarModal" data-bs-whatever="<?=$orc_prod[0]['cod']?>">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class=""  action="../Controller/AcoesProduto.php" method="POST">
          ...
          <div class="row">
            <div class="col-6">
              ...
              <input type="text" class="form-control" id="recipient-modelo" name="modelo" readonly="readonly">
            </div>
            <div class="col-6">
              <label for="recipient-email" class="col-form-label">Fabricante:</label>
              <input type="text" class="form-control" id="recipient-fab" name="fabricante" readonly="readonly">
            </div>
          </div>
          <div class="col-12">
            <label for="recipient-email" class="col-form-label">Descrição</label>
            <textarea class="form-control" id="recipient-descricao" name="descricao"  readonly="readonly"></textarea>
          </div>
           <div class="col-12">
          ...
          <div class="row">
            <div class="col-6">
              ...
              <input type="text" class="form-control" id="recipient-estado" name="estado" readonly="readonly">
            </div>
            <div class="col-6">
              <label for="recipient-acesso" class="col-form-label">Status:</label>
              <select class="form-select" aria-label="Default select example" id="recipient-sttsp" name="stts">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary" name="enviar" value="editarProdutos">Enviar</button>
        </div>
      </form>
    </div>
  </div>
...
<div class="modal fade" id="visualizarOrc" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">New message</h5>
      ...
      <div class="modal-body">
        <form class=""  action="../Controller/AcoesOrcamento_prod.php" method="POST">
          <div class="col-12">
              <label for="recipient-nome" class="col-form-label">Cliente:</label>
              <input type="text" class="form-control" id="recipient-cliente" name="data" readonly="readonly">
            </div>
          <div class="row">
            <div class="col-3">
          ...
          <div class="col-12">
            <label for="recipient-email" class="col-form-label">Descrição</label>
            <textarea class="form-control" id="recipient-desc" name="descricao"  readonly="readonly"></textarea>
          </div>
          <div class="col-12">
              <label for="recipient-nome" class="col-form-label">Valor:</label>
              <input type="text" class="form-control" id="recipient-valor" name="data" readonly="readonly">
            </div>
        </form>
      </div>
    </div>
...
<script type="text/javascript">
  ...
  responderModal.addEventListener('show.bs.modal', function (event) {
                  ...
                  modalTitle.textContent = 'Editar Produto: ' + recipientcod
              ...
              <script type="text/javascript">
  var visualizarOrc = document.getElementById('visualizarOrc')
  visualizarOrc.addEventListener('show.bs.modal', function (event) {
                  // Button that triggered the modal
                  var button = event.relatedTarget
                  // Extract info from data-bs-* attributes
                  var recipientcod = button.getAttribute('data-bs-whatever');
                  ...
                  var recipientcliente = button.getAttribute('data-bs-whateverclliente');
                  ...
                  //
                  // Update the modal's content.
                  var modalTitle = visualizarOrc.querySelector('.modal-title')
                  var modal = $(this)

                  modalTitle.textContent = 'Detalhes do Orçamento: ' + recipientcod
                  modal.find('#recipient-cod').val(recipientcod)
              ...
              <script type="text/javascript">
                document.getElementById('item6').style.color = 'white';
              </script>
            </body>
            </html>
